...
// Compatibility layer is included separately where needed to avoid namespace conflicts
...
// Boolean flags - REMOVED: now use global_simulation.assume_separate() etc.
...
// extern bool weight_window_checkpoint_surface;   //!< enable weight window check upon surface crossing?
// extern bool weight_window_checkpoint_collision; //!< enable weight window check upon collision?
...
// Paths to various files - REMOVED: now use global_simulation.path_input() etc.
...
// extern std::string weight_windows_file;   //!< Location of weight window file to load on simulation initialization
...
// Integer settings - REMOVED: now use global_simulation.n_inactive() etc.
...
// extern "C" int32_t max_write_lost_particles;       //!< maximum number of lost particles to be written to files
// extern "C" int32_t gen_per_batch; //!< number of generations per batch
// extern "C" int64_t n_particles;   //!< number of particles per generation

// Additional settings - REMOVED: now use global_simulation methods
...
// extern array<double, 4> energy_cutoff; //!< Energy cutoff in [eV] for each particle type
// extern array<double, 4> time_cutoff; //!< Time cutoff in [s] for each particle type
// extern int ifp_n_generation; //!< Number of generation for Iterated Fission Probability
// extern IFPParameter ifp_parameter; //!< Parameter to calculate for Iterated Fission Probability
...
// extern vector<std::string> res_scat_nuclides;           //!< Nuclides using res. upscattering treatment
...
// extern double source_rejection_fraction; //!< Minimum fraction of source sites that must be accepted

// extern int max_history_splits; //!< maximum number of particle splits for weight windows
// extern int64_t ssw_max_particles; //!< maximum number of particles to be banked on surfaces per process
// extern int64_t ssw_max_files;     //!< maximum number of surface source files to be created
// extern int64_t ssw_cell_id;       //!< Cell id for the surface source write setting
// extern SSWCellType ssw_cell_type; //!< Type of option for the cell argument of surface source write
...
// extern array<double, 2> temperature_range;           //!< Min/max T in [K] over which to load xs
...
// extern vector<array<int, 3>> track_identifiers;               //!< Particle numbers for writing tracks
