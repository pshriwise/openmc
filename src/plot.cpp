...
void PhongRay::on_intersection()
{
  ...
  if (!reflected_) {
    ...
    // If the light is colocated with the camera, any visible point should not
    // cast additional shadows along the camera ray.
    if ((plot_.light_location_ - plot_.camera_position()).norm() <
        100.0 * TINY_BIT) {
      stop();
      return;
    }
    ...
    // Shadow-ray bias to avoid self-occlusion on coincident/near-coincident
    // surfaces after advancing through the first-hit boundary.
    Direction bias_dir = (normal.dot(to_light) >= 0.0) ? normal : -normal;
    r() = r_hit + 100.0 * TINY_BIT * bias_dir;
