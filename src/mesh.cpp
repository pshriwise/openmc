  ...
  double l) const
{
  ...
  if (i == 0) {
    // create temporary position to determine which crossing to select
    if ((r0.x * r0.x + r0.y * r0.y) == 0.0 || (r0.x * u.x + r0.y * u.y) > 0.0) {
      return MeshDistance(ijk[i] + 1, true, find_r_crossing(r0, u, l, ijk[i]));
    } else {
      return MeshDistance(
        ijk[i] - 1, false, find_r_crossing(r0, u, l, ijk[i] - 1));
    }
